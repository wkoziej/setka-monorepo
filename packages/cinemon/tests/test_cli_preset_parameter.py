...
class TestCLIPresetParameter:
    ...
    def test_parse_args_requires_preset_or_config(self):
        ...
        with patch.object(sys, 'argv', ["blend_setup.py"] + test_args):
            with pytest.raises(SystemExit):  # argparse exits when required group not provided
                parse_args()
    ...
    def test_main_preset_mode_logging(self, mock_get_logger, mock_load_yaml, mock_generator_class, mock_manager_class):
        ...
        assert result == 0
        preset_log_calls = [call for call in mock_logger.info.call_args_list if "preset" in str(call).lower()]
