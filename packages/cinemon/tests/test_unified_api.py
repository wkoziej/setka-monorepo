...
def sample_preset_config():
    ...
    return {
        "layout": {
            "type": "main-pip",
            "config": {"pip_scale": 0.25, "margin_percent": 0.08},
        },
        "strip_animations": {
            "Video_1": [
                {
...
class TestAnimationAPI:
    ...
    def test_apply_preset_with_config(
        self, mock_bpy, sample_preset_config, sample_audio_data
    ):
        """Test applying preset with pre-loaded configuration."""
        with (
            patch.object(
                AnimationAPI, "apply_layout", return_value=True
            ) as mock_layout,
            patch.object(
                AnimationAPI, "apply_animations", return_value=True
            ) as mock_animations,
    ...
    def test_apply_preset_with_preset_name(self, mock_bpy):
        ...
        with (
            patch.object(
                AnimationAPI, "_load_preset_config", return_value=mock_config
            ) as mock_load,
            patch.object(AnimationAPI, "apply_layout", return_value=True),
            patch.object(AnimationAPI, "apply_animations", return_value=True),
            patch("unified_api.bpy", mock_bpy),
        ):
            api = AnimationAPI()

            result = api.apply_preset(
                recording_path="/test/recording", preset_name="multi-pip"
            )
    ...
    def test_apply_preset_missing_parameters(self, mock_bpy):
        ...
        assert (
            "Either preset_name or preset_config must be provided" in result["errors"]
        )
    ...
    def test_apply_preset_no_strips(self, mock_bpy, sample_preset_config):
        ...
        with patch("unified_api.bpy", mock_bpy):
            ...
            result = api.apply_preset(
                recording_path="/test/recording", preset_config=sample_preset_config
            )
    ...
    def test_apply_layout_success(self, mock_bpy):
        ...
        layout_config = {"type": "main-pip", "config": {"pip_scale": 0.25}}

        with patch(
            "layout_applicators.apply_layout_to_strips", return_value=True
        ) as mock_apply:
            api = AnimationAPI()
    ...
    def test_apply_layout_failure(self, mock_bpy):
        ...
        with patch("layout_applicators.apply_layout_to_strips", return_value=False):
            api = AnimationAPI()
            video_strips = [MagicMock(), MagicMock()]
    ...
    def test_apply_animations_with_audio_data(self, mock_bpy, sample_audio_data):
        ...
        with patch("unified_api.bpy", mock_bpy):
            ...
            result = api.apply_animations(
                video_strips, animations_config, sample_audio_data
            )
    ...
    def test_convert_strip_animations_to_list(self, mock_bpy):
        ...
        strip_animations = {
            ...
            "Camera2": [{"type": "rotation", "trigger": "beat", "intensity": 0.5}],
        }
...
"""
        ...
        with (
            patch("builtins.open", mock_open(read_data=mock_preset_content)),
