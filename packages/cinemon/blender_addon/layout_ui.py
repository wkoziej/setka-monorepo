...
try:
    ...
    from vse.layouts.main_pip_layout import (
        MainPipLayout as GridLayout,  # Use MainPipLayout as Grid alternative
    )
...
class CINEMON_PT_layout_panel(Panel):
    ...
    def draw_random_params(self, layout, scene):
        ...
        box.operator("cinemon.set_random_params", text="Adjust Parameters")
    ...
    def draw_grid_params(self, layout, scene):
        ...
        # Parameter adjustment operator
        box.operator("cinemon.set_grid_params", text="Adjust Parameters")
...
class CINEMON_OT_set_layout_type(Operator):
    ...
    try:
        layout_type: StringProperty(
            name="Layout Type", description="Type of layout to use", default="random"
        )
    except NameError:
    ...
    def execute(self, context):
        ...
        try:
            ...
            self.report(
                {"INFO"}, f"Layout type set to: {self.layout_type} (pending Apply)"
            )
            return {"FINISHED"}
...
class CINEMON_OT_set_random_params(Operator):
    ...
    try:
        ...
        overlap_allowed: BoolProperty(
            name="Allow Overlap", description="Allow strips to overlap", default=False
        )
    except NameError:
        # For testing without bpy
        margin = 0.1
    ...
    def execute(self, context):
        ...
        try:
            ...
            manager.update_layout_parameter(
                "config.overlap_allowed", self.overlap_allowed
            )

            self.report({"INFO"}, "Random layout params set (pending Apply)")
            return {"FINISHED"}
        except Exception as e:
...
class CINEMON_OT_set_grid_params(Operator):
    ...
    try:
        rows: IntProperty(
            name="Rows", description="Number of rows in grid", default=2, min=1, max=4
        )
    ...
    def execute(self, context):
        ...
        try:
            ...
            self.report(
                {"INFO"}, f"Grid layout set: {self.rows}x{self.columns}, gap={self.gap}"
            )
            return {"FINISHED"}
        except Exception as e:
...
class CINEMON_OT_preview_layout(Operator):
    ...
    def execute(self, context):
        ...
        try:
            ...
            for strip, position in zip(video_strips, positions):
                ...
                if hasattr(strip, "transform"):
                    ...
                    x, y, scale_x, scale_y = (
                        position[:4]
                        if len(position) >= 4
                        else (position[0], position[1], 1.0, 1.0)
                    )
                    ...
                    print(
                        f"Set strip '{strip.name}' to position ({x}, {y}) scale ({scale_x}, {scale_y})"
                    )
                else:
            ...
            self.report(
                {"INFO"},
                f"Applied {layout_type} layout preview to {len(video_strips)} strips",
            )
            return {"FINISHED"}
