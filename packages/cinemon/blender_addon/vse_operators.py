...
def regenerate_animations_for_strips(changed_strips: List[str], config: Dict[str, Any]) -> None:
    ...
    try:
        ...
        if not (bpy.context.scene.sequence_editor and bpy.context.scene.sequence_editor.sequences):
        ...
        try:
            ...
            if analysis_file:
                ...
                if not analysis_path.is_absolute():
                    ...
                    if recording_dir:
                        ...
                        if not analysis_path.exists():
                            print(f"Audio analysis file not found in recording dir. Tried: {[str(p) for p in test_paths]}")
                    else:
                ...
                if analysis_path.exists():
                    ...
                    print(f"Loaded audio events from {analysis_path}: {list(audio_events.keys())}")
                else:
                    print(f"Audio analysis file not found: {analysis_file} (resolved to: {analysis_path})")
        ...
        for sequence in sequences:
            if sequence.name in changed_strips:
                ...
                if sequence.name in strip_animations:
                    print(f"  ⚠ Strip '{sequence.name}' has animations but not in changed_strips!")

        print(f"DEBUG: changed_strips = {changed_strips}")
        print(f"DEBUG: strip_animations keys = {list(strip_animations.keys())}")
        print(f"Regenerating animations for {len(strips_to_process)} strips: {[s.name for s in strips_to_process]}")
        ...
        for strip in strips_to_process:
            ...
            print(f"DEBUG: Processing strip '{strip_name}' with {len(animations)} animations")

            if animations:
                print(f"Applying {len(animations)} animations to strip '{strip_name}'")
                apply_animation_to_strip(strip, animations, audio_events, fps)
            else:
                print(f"No animations for strip '{strip_name}', clearing existing keyframes")
    ...
    def draw_apply_status(self, layout) -> None:
        ...
        try:
            if self.has_pending_changes():
                ...
                box = layout.box()
                box.alert = True
                box.label(text=f"Pending changes: {len(changed_strips)} strips", icon='ERROR')
                ...
                box.operator("cinemon.discard_changes", text="Discard Changes", icon='X')
            else:
                # No pending changes
