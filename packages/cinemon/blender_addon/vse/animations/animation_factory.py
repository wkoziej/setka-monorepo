...
class AnimationFactory:
    ...
    def create(cls, animation_spec: Dict[str, Any]) -> Optional[BaseEffectAnimation]:
        ...
        try:
            ...
            target_info = f", target_strips={params['target_strips']}" if params['target_strips'] else ""
            print(f"🎨 Created {animation_type.capitalize()}Animation: trigger={params.get('trigger')}, intensity={params.get('intensity', 'N/A')}{target_info}")
            return animation
    ...
    @classmethod
    def create_multiple(cls, animation_specs: List[Dict[str, Any]]) -> List[BaseEffectAnimation]:
        """
