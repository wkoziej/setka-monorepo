...
try:
    import bpy
    from bpy.types import Panel, PropertyGroup, CollectionProperty
...
try:
    ...
    from .animation_ui import AnimationUIManager, AnimationPropertyGroup
...
class CINEMON_PT_active_strip_animations(Panel):
    ...
    def draw(self, context):
        ...
        try:
            ...
            if hasattr(manager, 'config') and 'strip_animations' in manager.config:
                ...
                print(f"DEBUG Animation Panel: No strip_animations in config or no config loaded")
    ...
    def draw_add_animation_section(self, layout):
        ...
        try:
            ...
            row = box.row()
            row.label(text="Type:")
            type_op = row.operator_menu_enum("cinemon.add_animation", property="animation_type", text="Select Type")
