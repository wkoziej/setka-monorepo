...
try:
    ...
    from .vse_operators import ApplyUIManager
except ImportError:
...
class CINEMON_PT_active_strip_animations(Panel):
    ...
    def draw(self, context):
        ...
        try:
            ...
            print(
                f"DEBUG Animation Panel: Strip '{active_strip_name}' has {len(animations)} animations"
            )

            # Debug: print loaded config
            if hasattr(manager, "config") and "strip_animations" in manager.config:
                print(
                    f"DEBUG Animation Panel: Available strips in config: {list(manager.config['strip_animations'].keys())}"
                )
            else:
                print(
                    "DEBUG Animation Panel: No strip_animations in config or no config loaded"
                )
        ...
        try:
            ...
            if "intensity" in animation:
                ...
                intensity_op = row.operator(
                    "cinemon.update_animation_param",
            ...
            if (
                ...
                duration_op = row.operator(
                    "cinemon.update_animation_param",
                    text=f"{animation['duration_frames']:.0f} frames",
                )
                ...
                return_op = row.operator(
                    "cinemon.update_animation_param",
                    text=f"{animation['return_frames']:.0f} frames",
                )
                ...
                if "contrast_boost" in animation:
                    ...
                    contrast_op = row.operator(
                        "cinemon.update_animation_param",
                        text=f"{animation['contrast_boost']:.2f}",
                    )
                ...
                if "grain_intensity" in animation:
                    ...
                    grain_op = row.operator(
                        "cinemon.update_animation_param",
                        text=f"{animation['grain_intensity']:.2f}",
                    )
    ...
    def draw_add_animation_section(self, layout):
        ...
        try:
            ...
            row.operator_menu_enum(
                "cinemon.add_animation", property="animation_type", text="Select Type"
            )
...
class CINEMON_OT_remove_animation(bpy.types.Operator):
    ...
    animation_index: IntProperty(
        name="Animation Index", description="Index of animation to remove"
    )
...
class CINEMON_OT_update_animation_param(bpy.types.Operator):
    ...
    animation_index: IntProperty(
        name="Animation Index", description="Index of animation to update"
    )

    param_name: StringProperty(
        name="Parameter Name", description="Name of parameter to update"
    )

    param_value: FloatProperty(
        name="Parameter Value", description="New parameter value"
    )

    # Override for input
    new_value: FloatProperty(
        name="New Value", description="Enter new value for parameter"
    )
    ...
    def execute(self, context):
        ...
        try:
            ...
            if manager.update_animation_parameter(
                self.animation_index, self.param_name, self.new_value
            ):
...
class CINEMON_OT_add_animation(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        try:
            ...
            new_animation = manager.create_default_animation(
