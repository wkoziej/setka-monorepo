...
Kanon faktów dla agentów specyficznych dla pakietu `beatrix`. Fakty ogólnomonorepowe
...
```bash
# Install dev dependencies for beatrix (workspace install `uv sync` — patrz root AGENTS.md)
uv sync --group dev
```
