    ...
    def _substitute_simple(text: str, variables: Dict[str, Any]) -> str:
        ...
        def replace_var(match):
            ...
            if var_name not in variables:
                raise TemplateError(
                    f"Missing template variable: {var_name}", variable_name=var_name
                )
    ...
    async def publish_post(
        ...
        if not self.is_authenticated:
            raise AuthenticationError(
                "Authentication required before publishing", platform=self.platform_name
            )
            ...
            try:
                if self.timeout > 0:
                    result = await asyncio.wait_for(
                        self._publish_post(
                            processed_content, metadata, progress_callback
                        ),
                ...
                if not self._is_retryable_error(e) or attempt == self.retry_attempts:
                    self.logger.error(
                        f"Publishing failed after {attempt + 1} attempts: {e}"
                    )
                    raise e

                # Log retry attempt
                self.logger.warning(
                    f"Publishing attempt {attempt + 1} failed, retrying: {e}"
                )
        ...
        if last_error:
            ...
            raise PublishError(
                "Publishing failed for unknown reason", platform=self.platform_name
            )
