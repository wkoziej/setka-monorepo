    ...
    def _make_api_request(
        ...
        try:
            ...
            try:
                ...
                raise NetworkError(
                    f"Invalid JSON response from Facebook API: {e}", platform="facebook"
                )
            ...
            raise NetworkError(
                f"Network error during API request: {e}", platform="facebook"
            )
        ...
        try:
            ...
            if not token_data.get("is_valid", False):
                ...
                raise AuthenticationError(
                    f"Invalid access token: {error_message}", platform="facebook"
                )
            ...
            raise NetworkError(f"Failed to validate token: {e}", platform="facebook")

    def check_permissions(
        self, required_permissions: Optional[List[str]] = None
    ) -> bool:
        """
        ...
        try:
            response_data = self._make_api_request(
                "GET", f"/{self.page_id}/permissions"
            )
            permissions_data = response_data.get("data", [])

            # Build permission status map
            permissions = {
                perm["permission"]: perm["status"] for perm in permissions_data
            }

            # Check for missing permissions
            missing_permissions = [
                perm for perm in required_permissions if perm not in permissions
            ]
            ...
            raise NetworkError(f"Failed to check permissions: {e}", platform="facebook")
    ...
    def get_long_lived_token(self) -> str:
        ...
        try:
            ...
            if not long_lived_token:
                raise AuthenticationError(
                    "No access token in exchange response", platform="facebook"
                )
            ...
            raise AuthenticationError(
                f"Failed to exchange token: {e}", platform="facebook"
            )
