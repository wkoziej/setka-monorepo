...
class FacebookPublisher(BasePublisher):
    ...
    async def authenticate(self) -> bool:
        ...
        try:
            ...
            if not self._auth.check_permissions():
                raise AuthenticationError(
                    "Insufficient Facebook permissions for posting",
                    platform="facebook"
                )
    ...
    def _validate_content(self, content: str, metadata: Dict[str, Any]) -> None:
        ...
        if "scheduled_publish_time" in metadata:
            ...
            if scheduled_time <= current_time:
                raise ValidationError(
                    "scheduled_publish_time must be in the future",
                    platform="facebook"
                )
    ...
    async def _publish_post(
        ...
        if self._auth is None:
            raise AuthenticationError(
                "Not authenticated with Facebook API",
                platform="facebook"
            )
        ...
        try:
            ...
            if "scheduled_publish_time" in metadata:
                result_metadata["scheduled_publish_time"] = metadata["scheduled_publish_time"]
            ...
            raise PublishError(
                f"Failed to publish post to Facebook: {e}",
                platform="facebook"
            ) from e
    
    def _format_post_data(self, content: str, metadata: Dict[str, Any]) -> Dict[str, Any]:
        """
        Format post data for Facebook API.
    ...
    def _extract_post_id(self, response: Dict[str, Any]) -> str:
        ...
        raise PublishError(
            "Invalid response format: no post ID found",
            platform="facebook"
        )
