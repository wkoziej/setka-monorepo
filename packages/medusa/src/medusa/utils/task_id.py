...
class TaskIDGenerator:
    ...
    def parse_task_id(self, task_id: str) -> Dict[str, Any]:
        ...
        if not isinstance(task_id, str):
            raise InvalidTaskIDError(
                f"Task ID must be string, got {type(task_id)}", task_id
            )
        ...
        if not uuid_match:
            raise InvalidTaskIDError(
                f"No valid UUID4 found in task ID: {task_id}", task_id
            )
        ...
        if not timestamp_match:
            raise InvalidTaskIDError(
                f"No valid timestamp found in task ID: {task_id}", task_id
            )
        ...
        if not before_timestamp.startswith(self.prefix):
            raise InvalidTaskIDError(
                f"Task ID prefix mismatch: expected '{self.prefix}', got prefix from '{task_id}'",
                task_id,
            )
        ...
        if after_prefix:
            if not after_prefix.startswith("_"):
                raise InvalidTaskIDError(
                    f"Invalid task ID format after prefix: {task_id}", task_id
                )
            ...
            if not self._validate_task_type(task_type):
                raise InvalidTaskIDError(
                    f"Invalid task type in task ID: {task_type}", task_id
                )

        # Validate timestamp format
        if not self._validate_timestamp(timestamp_str):
            raise InvalidTaskIDError(
                f"Invalid timestamp format in task ID: {timestamp_str}", task_id
            )
        ...
        try:
            ...
            raise InvalidTaskIDError(
                f"Invalid timestamp value in task ID: {timestamp_str}", task_id
            ) from e

        # Validate and parse UUID
        if not self.UUID4_PATTERN.match(uuid_str):
            raise InvalidTaskIDError(
                f"Invalid UUID format in task ID: {uuid_str}", task_id
            )
        ...
        try:
            ...
            if uuid_obj.version != 4:
                raise InvalidTaskIDError(
                    f"Task ID must use UUID4, got UUID{uuid_obj.version}", task_id
                )
        except ValueError as e:
            raise InvalidTaskIDError(
                f"Invalid UUID in task ID: {uuid_str}", task_id
            ) from e
    ...
    @classmethod
    def quick_generate(
        cls, prefix: str = DEFAULT_PREFIX, task_type: Optional[str] = None
    ) -> str:
        """
