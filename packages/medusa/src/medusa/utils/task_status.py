...
class TaskStatusManager:
    ...
    def get_multiple_task_statuses(
        self,
        task_ids: List[str],
    ...
    def query_task_statuses(
        ...
        with self._lock:
            ...
            if query.created_after:
                all_tasks = [
                    t for t in all_tasks if t.created_at >= query.created_after
                ]

            if query.created_before:
                all_tasks = [
                    t for t in all_tasks if t.created_at <= query.created_before
                ]
            ...
            for task in all_tasks:
                ...
                if include_history:
                    try:
                        ...
                        logger.warning(
                            f"Failed to get history for task {task.task_id}: {e}"
                        )
    ...
    def get_task_performance_metrics(self, task_id: str) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            total_duration = (
                task_result.updated_at - task_result.created_at
            ).total_seconds()
