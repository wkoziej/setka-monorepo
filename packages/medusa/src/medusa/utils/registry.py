    ...
    def validate(self) -> None:
        ...
        if not self.name.replace("_", "").isalnum():
            raise ValidationError(
                "Platform name must contain only alphanumeric characters and underscores"
            )
...
class PlatformRegistry:
    ...
    def register_platform(
        self, platform_info: PlatformInfo, force: bool = False
    ) -> None:
        """
    ...
    def create_platform_instance(
        self, platform_name: str, config: PlatformConfig
    ...
    def check_platform_health(self, platform_name: str, config: PlatformConfig) -> bool:
        ...
        try:
            ...
            self.logger.warning(
                f"Health check failed for platform '{platform_name}': {e}"
            )
            return False
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        """
        uploaders = sum(
            1 for p in self._platforms.values() if p.platform_type == "uploader"
        )
        publishers = sum(
            1 for p in self._platforms.values() if p.platform_type == "publisher"
        )
    ...
    def _validate_platform_implementation(self, platform_info: PlatformInfo) -> None:
        ...
        if platform_info.platform_type == "uploader":
            ...
            raise ValidationError(
                f"Unknown platform type: {platform_info.platform_type}"
            )

        self.logger.debug(
            f"Validated implementation for platform '{platform_info.name}'"
        )
