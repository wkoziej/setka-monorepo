...
class TaskStore:
    ...
    def __init__(
        ...
        logger.debug(
            f"TaskStore initialized with cleanup_enabled={cleanup_enabled}, "
    ...
    def get_tasks_created_after(self, cutoff_time: datetime) -> List[TaskResult]:
        ...
        with self._lock:
            return [
                task for task in self._tasks.values() if task.created_at > cutoff_time
            ]
    ...
    def cleanup_old_tasks(
        ...
        if cleaned_count > 0:
            logger.info(
                f"Cleaned up {cleaned_count} old tasks (older than {max_age_hours}h)"
            )
    ...
    def _schedule_cleanup(self) -> None:
        ...
        self._cleanup_timer = threading.Timer(
            self.cleanup_interval_minutes * 60, cleanup_task
        )
        self._cleanup_timer.daemon = True  # Don't prevent program exit
        self._cleanup_timer.start()

        logger.debug(
            f"Scheduled automatic cleanup in {self.cleanup_interval_minutes} minutes"
        )
