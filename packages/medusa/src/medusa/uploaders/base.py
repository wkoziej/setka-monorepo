    ...
    async def upload_media(
        ...
        if not self.is_authenticated:
            raise AuthenticationError(
                "Authentication required before uploading", platform=self.platform_name
            )
        ...
        for attempt in range(self.retry_attempts + 1):
            try:
                ...
                if not self._is_retryable_error(e) or attempt == self.retry_attempts:
                    self.logger.error(
                        f"Upload failed after {attempt + 1} attempts: {e}"
                    )
                    raise e
        ...
        if last_error:
            ...
            raise UploadError(
                "Upload failed for unknown reason", platform=self.platform_name
            )
