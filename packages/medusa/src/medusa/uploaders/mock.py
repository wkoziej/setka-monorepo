...
class MockConfig:
    ...
    error_type: str = (
        "upload"  # "upload", "network", "rate_limit", "auth", "validation"
    )
...
class MockUploader(BaseUploader):
    ...
    async def _upload_media(
        ...
        if self.mock_config.simulate_progress and progress_callback:
            ...
            for step in range(self.mock_config.progress_steps + 1):
                bytes_uploaded = int(
                    (step / self.mock_config.progress_steps) * total_bytes
                )
                ...
                if step < self.mock_config.progress_steps:
                    await asyncio.sleep(
                        self.mock_config.upload_delay / self.mock_config.progress_steps
                    )
        else:
    ...
    async def cleanup(self) -> None:
        ...
        call_info = {"method": "cleanup", "timestamp": datetime.now(timezone.utc)}
        self.call_log.append(call_info)
