...
class YouTubeAuth:
    ...
    def validate_files(self) -> None:
        ...
        try:
            ...
            raise ConfigError(f"Error checking file existence: {e}", original_error=e)
    ...
    def save_credentials(self) -> None:
        ...
        if not self.credentials:
            raise AuthenticationError(
                "No credentials to save", platform=self.platform_name
            )

        if not self.credentials_file:
            raise ConfigError(
                "Credentials file not configured", missing_fields=["credentials_file"]
            )
        ...
        try:
            ...
            if self.load_existing_credentials():
                ...
                if self.validate_credentials():
                    ...
                    self.logger.info(
                        "Authentication successful with existing credentials"
                    )
                    return True
                ...
                if self.refresh_token():
                    if self.validate_credentials():
                        ...
                        self.is_authenticated = True
                        self.logger.info(
                            "Authentication successful after token refresh"
                        )
                        return True
