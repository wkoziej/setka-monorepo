...
class YouTubeUploader(BaseUploader):
    ...
    def _validate_metadata(self, metadata: MediaMetadata) -> None:
        ...
        if not metadata.title or not metadata.title.strip():
            raise ValidationError(
                "Title is required for YouTube uploads", platform=self.platform_name
            )
        ...
        if metadata.privacy and metadata.privacy not in [
            "public",
            "private",
            "unlisted",
        ]:
        ...
        if (
            metadata.default_audio_language
            and len(metadata.default_audio_language) > 10
        ):
            raise ValidationError(
    ...
    def _validate_file(self, file_path: str) -> None:
        ...
        if not os.path.exists(file_path):
            raise ValidationError(
                f"Video file not found: {file_path}", platform=self.platform_name
            )
        ...
        if file_size == 0:
            raise ValidationError(
                f"Video file is empty: {file_path}", platform=self.platform_name
            )
    ...
    def _convert_metadata_to_youtube_format(
        ...
        if metadata.scheduled_publish_time:
            status["publishAt"] = metadata.scheduled_publish_time.strftime(
                "%Y-%m-%dT%H:%M:%SZ"
            )
    ...
    async def upload_thumbnail(self, video_id: str, thumbnail_path: str) -> bool:
        ...
        try:
            ...
            media = MediaFileUpload(
                thumbnail_path, mimetype=self._get_thumbnail_mimetype(thumbnail_path)
            )
    ...
    async def _upload_media(
        ...
        try:
            ...
            response = await self._perform_resumable_upload(
                insert_request, progress_callback, file_size
            )
            ...
            if metadata.thumbnail_path:
                try:
                    thumbnail_uploaded = await self.upload_thumbnail(
                        video_id, metadata.thumbnail_path
                    )
                    self.logger.info(f"Thumbnail uploaded for video: {video_id}")
                except Exception as e:
                    self.logger.warning(
                        f"Thumbnail upload failed (continuing with video upload): {e}"
                    )
    ...
    async def _perform_resumable_upload(
        ...
        while response is None:
            try:
                ...
                if response is not None:
                    if "id" in response:
                        self.logger.info(
                            f"Video upload completed. ID: {response['id']}"
                        )
        ...
        raise UploadError(
            "YouTube upload failed unexpectedly", platform=self.platform_name
        )
