...
def create_facebook_config():
    ...
    return PlatformConfig(
        platform_name="facebook", credentials=FACEBOOK_CONFIG, enabled=True
    )
...
async def validate_credentials():
    ...
    try:
        ...
        if "authentication" in error_str or "access token" in error_str:
            ...
            print(
                "   • Użyj Graph API Explorer: me/accounts aby zobaczyć dostępne strony"
            )
...
if __name__ == "__main__":
    ...
    if FACEBOOK_CONFIG["page_id"] != "TWÓJ_PAGE_ID":
        ...
        print(
            "   3. Uruchom ponownie: uv run python examples/validate_facebook_credentials.py"
        )
        print("\n📖 Zobacz instrukcje w pliku FACEBOOK_SETUP.md")
