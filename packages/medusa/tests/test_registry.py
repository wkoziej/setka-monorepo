...
class TestPlatformInfo:
    ...
    def test_create_platform_info(self):
        ...
        info = PlatformInfo(
            ...
            capabilities=[PlatformCapability.VIDEO_UPLOAD, PlatformCapability.METADATA_SUPPORT],
            version="1.0.0",
    ...
    def test_platform_info_validation(self):
        ...
        with pytest.raises(ValidationError, match="Implementation class cannot be None"):
            PlatformInfo(
                name="test",
                display_name="Test",
                platform_type="uploader",
...
class TestPlatformCapability:
    ...
    def test_capability_values(self):
        ...
        for capability in expected_capabilities:
            ...
            assert isinstance(getattr(PlatformCapability, capability), PlatformCapability)
...
class TestPlatformRegistry:
    ...
    def sample_publisher_info(self):
        ...
        return PlatformInfo(
            ...
            platform_type="publisher",
            implementation_class=MockPublisher,
            capabilities=[PlatformCapability.TEXT_PUBLISHING, PlatformCapability.LINK_PUBLISHING],
            version="1.0.0",
            description="Facebook publisher"
        )
    ...
    def test_register_platform_duplicate(self, registry, sample_uploader_info):
        ...
        with pytest.raises(RegistryError, match="Platform 'youtube' is already registered"):
            registry.register_platform(sample_uploader_info)
    ...
    def test_list_platforms(self, registry, sample_uploader_info, sample_publisher_info):
    ...
    def test_list_platforms_by_type(self, registry, sample_uploader_info, sample_publisher_info):
    ...
    def test_list_platforms_by_capability(self, registry, sample_uploader_info, sample_publisher_info):
        """Test listing platforms filtered by capability."""
        registry.register_platform(sample_uploader_info)
        registry.register_platform(sample_publisher_info)
        
        video_platforms = registry.list_platforms(capability=PlatformCapability.VIDEO_UPLOAD)
        assert len(video_platforms) == 1
        assert video_platforms[0].name == "youtube"
        
        text_platforms = registry.list_platforms(capability=PlatformCapability.TEXT_PUBLISHING)
        assert len(text_platforms) == 1
        assert text_platforms[0].name == "facebook"
        
        # Capability not supported by any platform
        batch_platforms = registry.list_platforms(capability=PlatformCapability.BATCH_OPERATIONS)
    ...
    def test_create_platform_instance_unregistered(self, registry):
        ...
        with pytest.raises(RegistryError, match="Platform 'nonexistent' is not registered"):
    ...
    def test_validate_platform_implementation(self, registry):
        ...
        with pytest.raises(ValidationError, match="Uploader implementation must inherit from BaseUploader"):
        ...
        with pytest.raises(ValidationError, match="Publisher implementation must inherit from BasePublisher"):
    ...
    def test_check_platform_health(self, registry, sample_uploader_info):
        ...
        with patch.object(MockUploader, 'health_check', return_value=True) as mock_health:
            config = PlatformConfig(platform_name="youtube")
        ...
        with patch.object(MockUploader, 'health_check', side_effect=Exception("Health check failed")):
            is_healthy = registry.check_platform_health("youtube", config)
            assert is_healthy is False
    ...
    def test_discover_platforms_success(self, mock_importlib, registry):
        ...
        with patch('medusa.utils.registry.pkgutil.iter_modules') as mock_iter:
            mock_iter.return_value = [
                (None, "discovered_platform", False)
            ]
    ...
    def test_discover_platforms_no_platform_info(self, mock_importlib, registry):
        ...
        with patch('medusa.utils.registry.pkgutil.iter_modules') as mock_iter:
            mock_iter.return_value = [
                (None, "no_platform_info", False)
            ]
            
            discovered = registry.discover_platforms(["test.package"])
            assert len(discovered) == 0
    
    def test_clear_registry(self, registry, sample_uploader_info, sample_publisher_info):
    ...
    def test_get_registry_stats(self, registry, sample_uploader_info, sample_publisher_info):
    ...
    def test_export_registry_config(self, registry, sample_uploader_info, sample_publisher_info):
...
class TestIntegrationScenarios:
    ...
    def test_capability_based_platform_selection(self, populated_registry):
        ...
        metadata_platforms = registry.list_platforms(capability=PlatformCapability.METADATA_SUPPORT)
        ...
        for platform_config in config["platforms"]:
            if platform_config["name"] == "youtube":
                info = PlatformInfo(
                    ...
                    implementation_class=MockUploader,
                    capabilities=[PlatformCapability[cap] for cap in platform_config["capabilities"]],
