...
class TestTaskStatusManager:
    ...
    def test_init_with_custom_dependencies(self):
        ...
        manager = TaskStatusManager(
            task_store=mock_task_store, state_manager=mock_state_manager
        )
    ...
    def test_get_task_status_failed_with_error(self):
        ...
        assert (
            ...
            == "YouTube API error: Video file too large (max 128GB)"
        )
    ...
    def test_get_task_status_with_history(self):
        ...
        mock_history.transitions = [
            ...
            StateTransition(
                TaskState.PENDING, TaskState.IN_PROGRESS, datetime.now(timezone.utc)
            ),
            StateTransition(
                TaskState.IN_PROGRESS, TaskState.COMPLETED, datetime.now(timezone.utc)
            ),
        ]
    ...
    def test_query_tasks_by_time_range(self):
        ...
        tasks = [
            TaskResult(
                task_id="old_task", status=TaskStatus.COMPLETED, created_at=old_time
            ),
    ...
    def test_format_status_response_completed(self):
        ...
        assert (
            response_dict["results"]["youtube_url"]
...
class TestTaskStatusQuery:
    ...
    def test_query_validation(self):
        ...
        now = datetime.now(timezone.utc)
        with pytest.raises(ValueError):
            TaskStatusQuery(created_after=now, created_before=now - timedelta(hours=1))
...
class TestTaskStatusResponse:
    ...
    def test_response_initialization(self):
        ...
        response = TaskStatusResponse(task_id="test_task", status="completed")
    ...
    def test_response_to_dict_in_progress(self):
        ...
        response = TaskStatusResponse(
            task_id="test_task", status="in_progress", message="Processing..."
        )
    ...
    def test_response_to_dict_with_history(self):
        ...
        response = TaskStatusResponse(
            task_id="test_task", status="completed", history=history
        )
    ...
    def test_response_spec_compliance_in_progress(self):
        ...
        response = TaskStatusResponse(
            task_id="test_task", status="in_progress", message="Uploading to YouTube..."
        )
