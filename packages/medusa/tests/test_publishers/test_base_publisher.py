...
class TestPublishProgress:
    ...
    def test_publish_progress_validation(self):
        ...
        with pytest.raises(
            ValidationError, match="current_step cannot exceed total_steps"
        ):
...
class TestPublishResult:
    ...
    def test_publish_result_validation(self):
        ...
        with pytest.raises(
            ValidationError, match="Failed publishes must have error information"
        ):
...
class TestTemplateSubstitution:
    ...
    def test_template_substitution_multiple_variables(self):
        ...
        result = TemplateSubstitution.substitute(template, variables)
        assert (
            result == "New video: https://youtube.com/watch?v=abc123 - Duration: 5 mins"
        )
    ...
    def test_template_substitution_conditional(self):
        ...
        result = TemplateSubstitution.substitute(template, variables)
        assert (
            result == "Video available on YouTube: https://youtube.com/watch?v=abc123"
        )
...
class MockPublisher(BasePublisher):
    ...
    def __init__(
        self, platform_name: str = "mock", config: Optional[PlatformConfig] = None
    ):
    ...
    async def _publish_post(
        self, content: str, metadata: Dict[str, Any], progress_callback=None
    ) -> PublishResult:
        ...
        if progress_callback:
            progress_callback(
                PublishProgress(step="preparing", current_step=1, total_steps=3)
            )
            progress_callback(
                PublishProgress(step="uploading", current_step=2, total_steps=3)
            )
            progress_callback(
                PublishProgress(step="finalizing", current_step=3, total_steps=3)
            )
...
class TestBasePublisher:
    ...
    async def test_process_template_error_handling(self, mock_publisher):
        ...
        with patch(
            "medusa.publishers.base.TemplateSubstitution.substitute",
    ...
    async def test_is_retryable_error(self, mock_publisher):
        ...
        assert (
            mock_publisher._is_retryable_error(
        ...
        assert (
            mock_publisher._is_retryable_error(ValidationError("Invalid data")) is False
        )
