...
class TestTaskStore:
    ...
    def create_sample_task_result(
        self, task_id: str = None, status: TaskStatus = TaskStatus.PENDING
    ) -> TaskResult:
...
class TestTaskStoreQuerying:
    ...
    def test_get_tasks_by_status(self):
        ...
        expected_pending = sum(
            1 for task in self.sample_tasks if task.status == TaskStatus.PENDING
        )
        expected_completed = sum(
            1 for task in self.sample_tasks if task.status == TaskStatus.COMPLETED
        )
        ...
        def store_tasks():
            try:
                for i in range(tasks_per_thread):
                    ...
                    result = self.task_store.store_task(task)
                    self.results.append(
                        (threading.current_thread().name, task.task_id, result)
                    )
...
class TestTaskStoreCleanup:
    ...
    def test_cleanup_respects_status_filter(self):
        ...
        cleaned_count = self.task_store.cleanup_old_tasks(
            max_age_hours=24, status_filter=[TaskStatus.COMPLETED, TaskStatus.FAILED]
        )
    ...
    def test_automatic_cleanup_scheduling(self, mock_timer):
        ...
        TaskStore(cleanup_enabled=True, cleanup_interval_minutes=30)
