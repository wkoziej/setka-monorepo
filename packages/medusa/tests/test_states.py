...
class TestStateHistory:
    ...
    def test_get_state_duration(self):
        ...
        history.add_transition(
            StateTransition(TaskState.PENDING, TaskState.IN_PROGRESS, timestamp=t1)
        )
        history.add_transition(
            StateTransition(TaskState.IN_PROGRESS, TaskState.COMPLETED, timestamp=t2)
        )
    ...
    def test_rollback_to_previous_state(self):
        ...
        rollback_transition = history.rollback_to_state(
            TaskState.IN_PROGRESS, "Rolling back from failure"
        )
...
class TestTaskStateManager:
    ...
    def test_transition_state(self):
        ...
        transition = manager.transition_state(
            "task_123", TaskState.IN_PROGRESS, "Starting execution"
        )
    ...
    def test_transition_invalid_state(self):
        ...
        with pytest.raises(StateTransitionError):
            manager.transition_state(
                "task_123", TaskState.PENDING, "Invalid transition"
            )
    ...
    def test_rollback_task_state(self):
        ...
        rollback_transition = manager.rollback_task(
            "task_123", TaskState.IN_PROGRESS, "Retrying after failure"
        )
...
class TestTaskStateManagerIntegration:
    ...
    def test_task_failure_and_rollback(self):
        ...
        manager.rollback_task(
            "failing_task", TaskState.IN_PROGRESS, "Retrying after network recovery"
        )
        manager.transition_state(
            "failing_task", TaskState.COMPLETED, "Retry successful"
        )
        ...
        assert (
            len(history.transitions) == 5
        )  # init, progress, failed, rollback, completed
