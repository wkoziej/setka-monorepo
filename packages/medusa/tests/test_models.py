...
from medusa.models import (
    ...
    PublishRequest, TaskTransition, validate_task_transition
)
...
class TestTaskTransition:
    ...
    def test_valid_transitions(self):
        ...
        # From PENDING
        assert validate_task_transition(TaskStatus.PENDING, TaskStatus.IN_PROGRESS) is True
        assert validate_task_transition(TaskStatus.PENDING, TaskStatus.CANCELLED) is True
        
        # From IN_PROGRESS
        assert validate_task_transition(TaskStatus.IN_PROGRESS, TaskStatus.COMPLETED) is True
        assert validate_task_transition(TaskStatus.IN_PROGRESS, TaskStatus.FAILED) is True
        assert validate_task_transition(TaskStatus.IN_PROGRESS, TaskStatus.CANCELLED) is True
        
        # Terminal states should not transition
        assert validate_task_transition(TaskStatus.COMPLETED, TaskStatus.COMPLETED) is True
        assert validate_task_transition(TaskStatus.FAILED, TaskStatus.FAILED) is True
        assert validate_task_transition(TaskStatus.CANCELLED, TaskStatus.CANCELLED) is True
    
    def test_invalid_transitions(self):
        """Test invalid task state transitions."""
        # Cannot go backwards
        assert validate_task_transition(TaskStatus.IN_PROGRESS, TaskStatus.PENDING) is False
        assert validate_task_transition(TaskStatus.COMPLETED, TaskStatus.PENDING) is False
        assert validate_task_transition(TaskStatus.COMPLETED, TaskStatus.IN_PROGRESS) is False
        ...
        assert validate_task_transition(TaskStatus.CANCELLED, TaskStatus.IN_PROGRESS) is False
...
class TestTaskResult:
    ...
    def test_task_result_add_platform_result(self):
        ...
        result.add_platform_result("youtube", {"url": "https://youtube.com/watch?v=123"})
...
class TestMediaMetadata:
    ...
    def test_media_metadata_sanitize_tags(self):
        ...
        metadata = MediaMetadata(
            title="Tag Test",
            tags=["  spaced  ", "UPPERCASE", "special@chars!", ""]
        )
...
class TestPublishRequest:
    ...
    def test_publish_request_with_metadata(self):
        ...
        metadata = {
            ...
            "facebook": {
                "message": "Check out my video: {youtube_url}"
            }
        }
    ...
    def test_publish_request_validation_file_not_found(self):
        ...
        request = PublishRequest(
            media_file_path="/path/to/nonexistent_video.mp4",
            platforms=["youtube"]
        )
