...
class YouTubeAPIMock:
    ...
    def insert(self, part: str = None, body: Dict[str, Any] = None, media_body=None):
        ...
        if self.rate_limited:
            raise Exception(
                f"429 Rate limit exceeded. Retry after {self.retry_after} seconds"
            )
    ...
    def post(self, url: str, data: Dict[str, Any] = None, **kwargs):
        ...
        if self.should_fail:
            if self.failure_reason == "auth_error":
                ...
                return MockAPIResponse(
                    400,
                    {"error": {"message": f"Post failed: {self.failure_reason}"}},
                    raise_for_status=True,
                )
...
class VimeoAPIMock:
    ...
    def post(self, url: str, data: Dict[str, Any] = None, **kwargs):
        ...
        if self.should_fail:
            if self.failure_reason == "auth_error":
                return MockAPIResponse(
                    401, {"error": "Invalid access token"}, raise_for_status=True
                )
            else:
                return MockAPIResponse(
                    400,
                    {"error": f"Upload failed: {self.failure_reason}"},
                    raise_for_status=True,
                )
        ...
        video_data = {
            ...
            "upload": {"upload_link": f"https://upload.vimeo.com/upload_{video_id}"},
        }
...
class TaskAssertions:
    ...
    @staticmethod
    def assert_task_completed_successfully(
        task: TaskResult, expected_platforms: List[str] = None
    ):
        ...
        if expected_platforms:
            for platform in expected_platforms:
                assert any(platform in key for key in task.results.keys()), (
                    f"Missing results for platform: {platform}"
                )

    @staticmethod
    def assert_task_failed(
        task: TaskResult, expected_platform: str = None, expected_error_type: str = None
    ):
        ...
        if expected_platform:
            assert task.failed_platform == expected_platform, (
                f"Expected failure on {expected_platform}, got {task.failed_platform}"
            )

        if expected_error_type:
            assert expected_error_type.lower() in task.error.lower(), (
                f"Expected error type {expected_error_type} in error: {task.error}"
            )
    ...
    def assert_task_in_progress(task: TaskResult, expected_platform: str = None):
        ...
        if expected_platform:
            ...
            assert current_platform == expected_platform, (
                f"Expected current platform {expected_platform}, got {current_platform}"
            )
...
class ExceptionAssertions:
    ...
    def assert_medusa_error(
        ...
        assert isinstance(exception, expected_type), (
            f"Expected {expected_type.__name__}, got {type(exception).__name__}"
        )
        assert isinstance(exception, MedusaError), (
            f"Expected MedusaError subclass, got {type(exception).__name__}"
        )

        if expected_platform:
            assert exception.platform == expected_platform, (
                f"Expected platform {expected_platform}, got {exception.platform}"
            )

        if expected_error_code:
            assert exception.error_code == expected_error_code, (
                f"Expected error code {expected_error_code}, got {exception.error_code}"
            )

    @staticmethod
    def assert_exception_chaining(
        exception: MedusaError, expected_original_type: type = None
    ):
        """Assert exception has proper chaining."""
        assert exception.original_error is not None, (
            "Expected original error in exception chain"
        )
...
class AsyncTestUtils:
    ...
    def create_async_mock_with_delay(
        return_value: Any = None, delay: float = 0.1
    ) -> AsyncMock:
...
class PerformanceTestUtils:
    ...
    def assert_execution_time(
        duration: float, max_duration: float, operation_name: str = "Operation"
    ):
    ...
    def create_performance_benchmark(
        func: Callable, iterations: int = 100
    ) -> Dict[str, float]:
