...
class TestUploadProgress:
    ...
    def test_upload_progress_validation(self):
        ...
        # Test bytes > total
        with pytest.raises(ValidationError, match="bytes_uploaded cannot exceed total_bytes"):
...
class TestUploadResult:
    ...
    def test_upload_result_validation(self):
        ...
        with pytest.raises(ValidationError, match="Failed uploads must have error information"):
...
class ConcreteUploader(BaseUploader):
    ...
    def __init__(self, platform_name: str = "test", config: Optional[PlatformConfig] = None):
    ...
    async def authenticate(self) -> bool:
        ...
        if self.should_fail_auth:
            raise AuthenticationError("Authentication failed", platform=self.platform_name)
...
class TestBaseUploader:
    ...
    async def test_upload_with_retry_logic(self):
        ...
        with patch.object(uploader, '_upload_media') as mock_upload:
            mock_upload.side_effect = [
                ...
                UploadResult(platform="test", upload_id="123", media_url="https://test.com/123")
            ]
    ...
    async def test_upload_retry_exhausted(self):
        ...
        with patch.object(uploader, '_upload_media') as mock_upload:
            mock_upload.side_effect = NetworkError("Persistent network error", platform="test")
    ...
    async def test_upload_no_retry_on_auth_error(self):
        ...
        with patch.object(uploader, '_upload_media') as mock_upload:
            mock_upload.side_effect = AuthenticationError("Token expired", platform="test")
    ...
    async def test_upload_no_retry_on_validation_error(self):
        ...
        with patch.object(uploader, '_upload_media') as mock_upload:
            mock_upload.side_effect = ValidationError("Invalid file format", platform="test")
    ...
    def test_is_retryable_error(self):
        ...
        assert uploader._is_retryable_error(RateLimitError("Rate limited", platform="test"))
        
        # Non-retryable errors
        assert not uploader._is_retryable_error(AuthenticationError("Auth failed", platform="test"))
        assert not uploader._is_retryable_error(ValidationError("Invalid data", platform="test"))
...
class TestBaseUploaderIntegration:
    ...
    async def test_concurrent_uploads(self):
        ...
        tasks = [
            uploader.upload_media(f"video_{i}.mp4", metadata)
            for i in range(3)
        ]
