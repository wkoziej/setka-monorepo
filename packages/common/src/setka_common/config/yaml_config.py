...
class YAMLConfigLoader:
    ...
    def convert_to_env_vars(self, config: BlenderYAMLConfig) -> Dict[str, str]:
        ...
        env_vars["BLENDER_VSE_VIDEO_FILES"] = ",".join(config.project.video_files)
        
        if config.project.main_audio:
            env_vars["BLENDER_VSE_MAIN_AUDIO"] = config.project.main_audio
        
        if config.project.output_blend:
            env_vars["BLENDER_VSE_OUTPUT_BLEND"] = config.project.output_blend
        
        if config.project.render_output:
            env_vars["BLENDER_VSE_RENDER_OUTPUT"] = config.project.render_output
        
        env_vars["BLENDER_VSE_FPS"] = str(config.project.fps)
        
        if config.project.resolution:
            env_vars["BLENDER_VSE_RESOLUTION_X"] = str(config.project.resolution["width"])
            env_vars["BLENDER_VSE_RESOLUTION_Y"] = str(config.project.resolution["height"])
        
        env_vars["BLENDER_VSE_BEAT_DIVISION"] = str(config.project.beat_division)
        
        # Audio analysis configuration
        if config.audio_analysis.file:
            env_vars["BLENDER_VSE_AUDIO_ANALYSIS_FILE"] = config.audio_analysis.file
        
        if config.audio_analysis.data:
            import json
            env_vars["BLENDER_VSE_AUDIO_ANALYSIS"] = json.dumps(config.audio_analysis.data)
        
        # Layout configuration
        env_vars["BLENDER_VSE_LAYOUT_TYPE"] = config.layout.type
        
        if config.layout.config:
            # Convert layout config to string format
            config_parts = []
            for key, value in config.layout.config.items():
                if isinstance(value, bool):
                    config_parts.append(f"{key}={str(value).lower()}")
                else:
                    config_parts.append(f"{key}={value}")
            env_vars["BLENDER_VSE_LAYOUT_CONFIG"] = ",".join(config_parts)
        
        # Animation configuration
        if config.animations:
            env_vars["BLENDER_VSE_ANIMATION_MODE"] = "compositional"
            # Animation config conversion would be complex, skipping for now
        else:
            env_vars["BLENDER_VSE_ANIMATION_MODE"] = "none"
